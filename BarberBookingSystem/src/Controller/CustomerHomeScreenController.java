...
/**
 * This class was created to interat with Dao classes and the screen for the customer
 * @author Jorge
 */
public class CustomerHomeScreenController  extends MouseAdapter implements ActionListener {
    ...
    private void makeAppointment() {
        ...
        if (!view.getDate().equals("") && view.getSlot() != null && !view.getSlot().equals("")) {
            ...
            java.sql.Time timeAux = java.sql.Time.valueOf(zone.getHour()+":"+zone.getMinute()+":"+zone.getSecond());

            if (timeAux.toLocalTime().isAfter(time)) {
                view.showInfoMsg("The slot cannot be chosen because time has already passed");
                return;
            }
